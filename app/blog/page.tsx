      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {blogs.map((blog) => {
          ...
          return (
            <div
              key={blog._id}
              className="group rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition bg-white/95"
            >
              {/* Image */}
              ...
              <div className="p-5">
                {blog.category && (
                  <Link href={`/blogs/category/${categorySlug}`}>
                    <p className="text-sm text-slate-600 font-semibold mb-2 hover:underline ">
                      {blog.category}
                    </p>
                  </Link>
                )}
                ...
                <Link href={`/blogs/${blog.slug}`}>
                  <h2 className="text-lg font-bold text-slate-800 line-clamp-2 hover:text-amber-600 transition">
                    {blog.title}
                  </h2>
                </Link>
