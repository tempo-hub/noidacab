...
export function JourneyInfo({ location, vehicle }: Props) {
  const whatsappUrl = `https://wa.me/918377809809?text=${encodeURIComponent(
    `Hello NoidaCab, I want to book a ${vehicle.name} cab for pickup in ${location.name}. Please confirm dispatch.`
  )}`;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-7xl">
          ...
          <p className="mt-2 max-w-7xl text-sm leading-6 text-slate-600 sm:text-base sm:leading-7">
            ...
            <span className="font-semibold text-slate-900">{location.name}</span> with a professional driver, transparent billing, and zero cancellation penalty.
          </p>
        </div>
            ...
            <div className="lg:col-span-4">
              ...
              <div className="mt-4 space-y-2 text-xs text-slate-600">
                <div className="flex items-center gap-2">
                  ...
                  <span>Doorstep pickup with GPS tracking</span>
                </div>
                <div className="flex items-center gap-2">
                  <Clock className="h-3.5 w-3.5 text-amber-600 shrink-0" />
                  <span>ETA: 10–15 mins inside {location.name}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-700">
                  Assigned fleet for <strong className="text-slate-950">{location.name}</strong>: {vehicle.name} ({vehicle.seats} Seats, {vehicle.luggage} Bags, Chilled AC)
                </span>
              </div>
              <span className="font-bold text-amber-700">Fixed Tariff • No Hidden Cost</span>
            </div>
          </div>
        ...
        <div className="mt-6 grid max-w-7xl gap-3 sm:grid-cols-3">
          {[
            { title: "Verified Chauffeur", desc: "Background-checked & polite local driver" },
            { title: `Sanitized ${vehicle.name}`, desc: "Clean AC cabin with ample boot space" },
            { title: "Transparent Billing", desc: "Pay only for actual kilometers driven" },
          ].map((item) => (
            <div
              key={item.title}
